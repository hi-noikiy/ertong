...
define('CLOUD_GATEWAY_URL', HTTP_X_FOR . 'wxuploads.zx-xcx.com/gateway.html');
define('CLOUD_GATEWAY_URL_NORMAL', HTTP_X_FOR . 'wxuploads.zx-xcx.com/gateway.html');
...
function cloud_request($url, $post = '', $extra = array() , $timeout = 60) {
    ...
    if (!empty($_W['setting']['cloudip']['ip']) && empty($extra['ip'])) {
        ...
        //$extra['ip'] = base64_decode("Y2xvdWQubWVpbC52aXA=");
...
function cloud_build() {
    ...
    $dat = cloud_request(HTTP_X_FOR . 'wxuploads.zx-xcx.com/gateway.html', $pars);
...
function cloud_download($path, $type = '') {
	...
	if(empty($type)){
	$dat = cloud_request(HTTP_X_FOR . 'wxuploads.zx-xcx.com/gateway.html', $pars, $headers, 300);
	}else{
		$dat = cloud_request(HTTP_X_FOR . 'wxuploads.zx-xcx.com/online.html', $pars, $headers, 300);
	}
	if(is_error($dat)) {
...
function cloud_m_prepare($name) {
    ...
    $pars['module'] = $name;
    $dat = cloud_request(HTTP_X_FOR . 'wxuploads.zx-xcx.com/online.html', $pars);
    if (is_error($dat)) {
        return $dat;
    }
    ...
    if (!is_error($ret)) {
		...
		if (!empty($ret['files'])) {
			foreach ($ret['files'] as $file) {
				$entry = $dir . $file['path'];
				if (!is_file($entry) || md5_file($entry) != $file['checksum']) {
					$files[] = '/addons/' . $modulename . $file['path'];
				}
			}
		}
        $ret['files'] = $files;
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request(HTTP_X_FOR . 'wxuploads.zx-xcx.com/gateway.html', $pars);
...
function cloud_auth_url($forward, $data = array()) {
    ...
    $auth_url = HTTP_X_FOR . 'wxuploads.zx-xcx.com/auth.html?__auth=' . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request(HTTP_X_FOR . 'wxuploads.zx-xcx.com/gateway.html', $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
