...
<div class="panel-body" style="font-size: 12px;">
    <div class="ibox-content">
        <form action="<?php  echo url('site/entry/'.$_GPC['do'],array('m'=>$_GPC['m'],'op'=>'article','version_id'=>$_GPC['version_id']));?>" id="searchform" method="post">
            ...
            <table class="table-striped table-bordered table-hover dataTables-example table">
                ...
                <tr>
                    ...
                    <td style="text-align:right;">
                        <a data-dismiss="modal" class="btn btn-info btn-xs choose" data-url="<?php  echo $item['url'];?>" data-id="<?php  echo $item['id'];?>" data-name="<?php  echo $item['name'];?>"><i class="fa fa-check-square-o"></i>选择</a>
                </tr>
                <?php  } } ?>
                </tbody>
            </table>
...
<script>
    $(function(){
        $(".choose").click(function(){
            if(window.parent.ad_objc!=""){
                var link=$(this).attr("data-url");
                var id=$(this).attr("data-id");
                $(window.parent.ad_objc).parent().prev().val(link+'&id='+id);
                $("#sort_close2",window.parent.document).click();
            }else{
                if(window.parent.objc!=""){
                    var link=$(this).attr("data-url");
                    var id=$(this).attr("data-id");
                    $(window.parent.objc).parent().prev().val(link+'&id='+id);
                    $("#sort_close",window.parent.document).click();
                }
                if(window.parent.obj!=""){
                    var link=$(this).attr("data-url");
                    var id=$(this).attr("data-id");
                    var share='../link/link?&url='+escape(link+'&id='+id);
                    $(window.parent.obj).parent().prev().val(share);
                    $("#select-close",window.parent.document).click();
                }
            }
        });
    })
