		...
		<tr ng-repeat="module in module_list" ng-if="module[support] == 2">
			...
			<td class="text-right">
				<?php  if(permission_check_account_user('see_module_manage_system_install')) { ?>
					...
					<a ng-if="module.cloud_id" ng-href="{{ 'http://wxuploads.zx-xcx.com/module/id/' + module.cloud_id + '.html' }}?do=auth&u=<?php  echo $_W['siteroot'];?>" target="_blank" class="btn btn-primary">查看详情</a>
					<?php  } ?>
					<?php  if($do == 'recycle') { ?>
						<?php  if(permission_check_account_user('see_module_manage_system_stop')) { ?>
...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<div class="user-head-info we7-padding-bottom">
		...
		<?php  if(permission_check_account_user('see_module_manage_system_shopinfo')) { ?>
		<a ng-if="isFounder == 1 && moduleinfo.cloud_mid != ''" ng-href="{{ 'http://wxuploads.zx-xcx.com/module/id/' + moduleinfo.cloud_mid + '.html' }}?do=auth&u=<?php  echo $_W['siteroot'];?>" target="_blank" class="btn btn-primary pull-right">查看详情</a>
		<?php  } ?>
	</div>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && upgradeInfo.from == 'cloud' && branch.displayorder >= upgradeInfo.site_branch.displayorder" ng-repeat="branch in upgradeInfo.branches">
		...
		<tr>
			...
			<td colspan="4" class="text-right">
				...
				<a href="http://wxuploads.zx-xcx.com/module/id/{{upgradeInfo.id}}.html" ng-if="branch.displayorder > upgradeInfo.site_branch.displayorder || (branch.displayorder == upgradeInfo.site_branch.displayorder && branch.id > upgradeInfo.site_branch.id)" class="color-default view-detail" target="_blank">查看分支详情</a>
			</td>
		</tr>
