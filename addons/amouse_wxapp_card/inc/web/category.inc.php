...
if ($op == 'display') {
    ...
    $cates = pdo_fetchall("SELECT * FROM ".tablename('amouse_wxapp_category')." $condition ORDER BY displayorder DESC , createtime DESC ", $params);
    ...
    if (checksubmit('submit')) {
        for ($i = 0; $i < count($_GPC['titles']); $i++) {
            ...
            if ($ids[$i] != NULL) {
                pdo_update("amouse_wxapp_category", $insert, array('id' => $ids[$i]));
            } else {
    ...
    load()->func('file');
    $id = intval($_GPC['id']);
    pdo_delete('amouse_wxapp_category', array('id' => $id));
