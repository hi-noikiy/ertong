...
if(checksubmit('submit')){
	...
	if($res){
		message('修改成功',$this->createWebUrl2('dlinactivity',array()),'success');
	}else{
		message('修改失败','','error');
	}
	}else{
	...
	if($res){
		message('新增成功',$this->createWebUrl2('dlinactivity',array()),'success');
	}else{
		message('新增失败','','error');
	}	
	}
...
include $this->template('web/dlinaddactivity');