...
$info=pdo_get('zhtc_store',array('id'=>$storeid));
...
//$typein=pdo_getall('zhtc_in',array('uniacid'=>$_W['uniacid']));
   ...
   if(checksubmit('submit')){
   	...
   	if($_GPC['user_name']){
   		$rst=pdo_get('zhtc_store',array('user_name'=>$_GPC['user_name'],'uniacid'=>$_W['uniacid'],'id !='=>$storeid));
   		if($rst){
   ...
   //	$data['storetype2_id']=$_GPC['storetype2_id'];
   	...
   	$res = pdo_update('zhtc_store', $data, array('id' => $storeid));
   	if($res){
   		message('编辑成功',$this->createWebUrl2('sjstoreinfo',array()),'success');
   	}else{
   		message('编辑失败','','error');
   	}
   ...
   include $this->template('web/sjstoreinfo');