...
//	}
	...
	if(!empty($record)) {
		...
		$account = pdo_fetch("SELECT * FROM " . tablename("yzcj_sun_account") . " WHERE status=2 AND uid=:uid ORDER BY id DESC LIMIT 1", array(':uid' => $record['uid']));
		if (!empty($account)) {
...
//		message("欢迎回来，{$record['username']}。", $forward);
		...
		pdo_update("yzcj_sun_account", $data, array('id' => $record['id']));
		//echo url('site/entry/stores', array('m' => 'yzcj_sun', 'city' => $cityname, 'do' => 'start'));die;
		message("欢迎回来，{$record['username']}！", url('site/entry/stores', array('m' => 'yzcj_sun', 'city' => $cityname, 'account_id'=>$account_id,'do' => 'start')), 'success');
	} else {
		if (empty($failed)) {
