...
//    团长明细
    ...
    public function outCSV(){
        ...
        $list = $getModel()
            ->field('t2.order_no as order_no,t3.name,t1.goods_name,t4.name as leader_name,t2.pay_amount as order_pay_amount,t2.coupon_money as order_coupon_money,t1.pay_amount,t1.delivery_fee,t1.coupon_money,t1.attr_names,t1.state,t1.num,t2.create_time as order_time,t1.order_id')
            ->select();
        ...
        $this->toCSV('用户订单明细表'.date('ymdhis').'.csv',['订单号','用户','商品名称','团长','订单支付金额','订单优惠金额','商品支付金额','配送费','商品优惠金额','规格','状态','数量','下单时间','商户支付单号'],json_decode(json_encode($list),true));
    }
    //    团长明细
    ...
    public function outCSV2(){
        ...
        $this->toCSV('团长订单明细表'.date('ymdhis').'.csv',['商品名称','规格','状态','数量','团长','电话','地址'],json_decode(json_encode($list),true));
    }
    //    获取choose页数据
...
//采购统计表
    ...
    public function get_list5(){
        $getModel = function (){
            ...
            $model = Ordergoods::where('store_id',$_SESSION['admin']['store_id'])
                ->where('state',2)
                ->where('check_state <> 1')
    ...
    public function outCSV5(){
        ...
        $list = $getModel()
            ->field('goods_name,attr_names,sum(num) as num')
            ->group('goods_name,attr_names')
            ->order('goods_name,attr_names')
            ->select();

        $this->toCSV('采购统计表'.date('ymdhis').'.csv',['商品名称','规格','数量'],json_decode(json_encode($list),true));
    }
    //配送统计表
