...
/*            $delivery_num = $goods['num'];
    ...
    public function getPinorders(){
        ...
        $list = Pinorder::with('goods')->where($query)->select();
        success_withimg_json($list);
    }
//    确认拼团商家配送
...
//    确认用户提取商品(核销)
    public function confirmUserGoodses(){
        ...
        success_json(Pinorder::getLastSql());
    }
...
/*    public function pinGetMyLeader(){
        $pin = Db::name('pluginkey')->where('name','拼团')->find();
        $user_id = input('request.user_id');
        $data = Leader::get(['user_id'=>$user_id],'User');
        if ($data){
            $data['is_leader'] = 1;
