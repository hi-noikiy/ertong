...
class OrderPayDataForm extends ApiModel
{
    ...
    public function rules()
    {
        return [
            [['pay_type'], 'required'],
            [['pay_type'], 'in', 'range' => ['ALIPAY', 'WECHAT_PAY', 'HUODAO_PAY', 'BALANCE_PAY','BAIDU_PAY']],
    ...
    public function search()
    {
        ...
        if ($this->order_id) { //单个订单付款
            ...
            if ($this->pay_type == 'BAIDU_PAY') {
                ...
                $setting = MpConfig::get($this->store_id);
                require dirname(\Yii::$app->BasePath).'/core/extensions/baidu-pay-php-sdk/Autoloader.php';

                /**
                 * 第二部分：生成签名 DEMO
                 */
                $requestParamsArr = array(
                    'appKey' => $setting->public_app_key,
                    'dealId' => $setting->deal_id,
                    'tpOrderId' => $this->order->order_no,
                    'totalAmount' => $this->order->pay_price * 100,
                );

                $rsas = new \NuomiRsaSign();
                $rsaSign = $rsas->genSignWithRsa($requestParamsArr, $setting->rsa_private_key);
                //-----BEGIN PUBLIC KEY----- -----END PUBLIC KEY----- -----BEGIN RSA PRIVATE KEY----- -----END RSA PRIVATE KEY-----
                $requestParamsArr['sign'] = $rsaSign;
                $checkSign = $rsas->checkSignWithRsa($requestParamsArr, $setting->rsa_public_key);
                if (!$checkSign) {
                    return [
                        'code' => 1,
                        'msg' => '签名错误.',
                ...
                $bizInfo = [
                    'tpData' => [
                        ...
                        'dealId' => $setting->deal_id,
                        'tpOrderId' => $this->order->order_no,
                        'totalAmount' => $this->order->pay_price,
                        'rsaSign' => $rsaSign,
                        'returnData' => [],
                        'displayData' => [],
                    ],
