var interval, api = require("../../api.js"),
    app = getApp();
Page({
    data: {
        isStart: !0,
    ...
    onShow: function() {
        ...
        app.request({
            ...
            success: function(t) {
                ...
                // a.title && swan.setNavigationBarTitle({
                //     title: a.title
                // }), 
                i.setData({
                    // title: a.title,
    ...
    register: function() {
        this.data.error ? swan.showModal({
            title: "提示",
    ...
    init: function() {
        var t = swan.createSelectorQuery(),
            s = this;
        s.setData({
            award_name: !1
            ...
            var i = swan.createCanvasContext("scratch");
    ...
    touchStart: function(t) {
        if (this.data.isStart) if (this.data.error) swan.showModal({
            title: "提示",
            content: this.data.error,
            showCancel: !1
        });
    ...
    touchEnd: function(t) {
        if (this.data.isStart && !this.data.error) {
            var e = this,
                a = this.data.canvasWidth,
                i = this.data.canvasHeight,
                s = this.data.minX,
                o = this.data.minY,
                r = this.data.maxX,
                n = this.data.maxY;.4 * a < r - s && .4 * i < n - o && this.data.detect && (e.setData({
                detect: !1
    ...
    onShareAppMessage: function() {
        return {
            path: "/scratch/index/index?user_id=" + swan.getStorageSync("user_info").id,
