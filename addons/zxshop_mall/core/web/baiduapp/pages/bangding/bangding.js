...
Page({
    ...
    getPhoneNumber: function(t) {
        var a = this;
        "getPhoneNumber:fail user deny" == t.detail.errMsg ? swan.showModal({
            title: "提示",
            showCancel: !1,
            content: "未授权",
            success: function(n) {}
                    ...
                    app.request({
                        ...
                        success: function(n) {
                            if (0 == n.code) {
                                ...
                                e.binding = n.data.dataObj, swan.setStorageSync("__user_info", e), a.setData({
    ...
    onSubmit: function() {
        ...
        if (n) {
            ...
            if (a) {
                if (11 != a.length) return void swan.showToast({
                    title: "手机号码错误",
                    image: "/images/icon-warning.png"
                });
                var i = a;
            } else {
                if (!(i = this.data.PhoneNumber)) return void swan.showToast({
                    title: "手机号码错误",
                    image: "/images/icon-warning.png"
                });
            }
            ...
            if (!/^\+?\d[\d -]{8,12}\d/.test(i)) return void swan.showToast({
                title: "手机号码错误",
                image: "/images/icon-warning.png"
            });
            var o = this.data.code;
            if (!o) return void swan.showToast({
                title: "请输入验证码",
                image: "/images/icon-warning.png"
            });
        }
