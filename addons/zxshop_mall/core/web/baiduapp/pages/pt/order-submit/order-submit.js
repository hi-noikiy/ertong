...
Page({
    ...
    onLoad: function(a) {
        app.pageOnLoad(this, a), swan.removeStorageSync("input_data");
    ...
    onShow: function() {
        app.pageOnShow(this);
        var a = this, t = swan.getStorageSync("picker_address");
        t && (a.setData({
            address: t,
            name: t.name,
            mobile: t.mobile
        }), swan.removeStorageSync("picker_address"), a.getInputData()), a.getOrderData(a.data.options);
    },
    onHide: function() {
    ...
    getOrderData: function(a) {
        var o = this, t = "";
        o.data.address && o.data.address.id && (t = o.data.address.id), a.goods_info && (swan.showLoading({
            title: "正在加载",
            mask: !0
        }), app.request({
            ...
            success: function(a) {
                if (swan.hideLoading(), 0 == a.code) {
                    ...
                    var i = swan.getStorageSync("input_data");
                    swan.removeStorageSync("input_data"), i || (i = {
    ...
    orderSubmit: function(a) {
        ...
        if (1 == (e.offline = i)) {
            if (!t.data.address || !t.data.address.id) return void swan.showToast({
                title: "请选择收货地址",
                image: "/images/icon-warning.png"
            });
            e.address_id = t.data.address.id;
        } else {
            if (e.address_name = t.data.name, e.address_mobile = t.data.mobile, !t.data.shop.id) return void swan.showToast({
                title: "请选择核销门店",
                image: "/images/icon-warning.png"
            });
            if (e.shop_id = t.data.shop.id, !e.address_name || null == e.address_name) return void swan.showToast({
                title: "请填写收货人",
                image: "/images/icon-warning.png"
            });
            if (!e.address_mobile || null == e.address_mobile) return void swan.showToast({
                title: "请填写联系方式",
                image: "/images/icon-warning.png"
            });
            if (!/^\+?\d[\d -]{8,12}\d/.test(e.address_mobile)) return void swan.showModal({
                title: "提示",
                content: "手机号格式不正确"
            });
        }
        ...
        swan.chooseLocation({
            ...
            fail: function(a) {
                app.getauth({
                    ...
                    success: function(a) {
                        a && (a.authSetting["scope.userLocation"] ? t.dingwei() : swan.showToast({
                            title: "您取消了授权",
                            image: "/images/icon-warning.png"
                        }));
                    }
    ...
    pickShop: function(a) {
        var t = swan.getStorageSync("input_data"), e = a.currentTarget.dataset.index;
    ...
    getInputData: function() {
        ...
        swan.setStorageSync("input_data", t);
    }
});