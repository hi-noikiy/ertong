...
Page({
    ...
    loadData: function(t) {
        var c = this, n = swan.getStorageSync("item");
        c.setData({
    ...
    onShareAppMessage: function(t) {
        var a = this;
        return {
            path: "/pages/quick-purchase/index/index?user_id=" + swan.getStorageSync("user_info").id,
            success: function(t) {
    ...
    jia: function(t) {
        ...
        for (var r in i) for (var s in i[r].goods) if (parseInt(i[r].goods[s].id) === parseInt(o.id)) {
            ...
            if (e > JSON.parse(i[r].goods[s].attr)[0].num) return void swan.showToast({
                title: "商品库存不足",
    ...
    attrClick: function(t) {
        ...
        for (var _ in n) {
            ...
            if (g.sort().join() === c.sort().join()) {
                if (0 === parseInt(n[_].num)) return void swan.showToast({
        ...
        if (i.length === o.length) {
            ...
            for (var d in e) {
                ...
                for (var n in e[d].attr_list) if (c.push([ e[d].attr_list[n].attr_id, r.id ]), c.sort().join() === i.sort().join()) {
                    ...
                    if (s > e[d].num) return void swan.showToast({
                        title: "商品库存不足",
                        image: "/images/icon-warning.png"
                    });
                }
