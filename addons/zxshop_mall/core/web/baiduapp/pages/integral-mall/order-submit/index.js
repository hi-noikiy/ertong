...
Page({
    ...
    onReady: function() {},
    onShow: function() {
        getApp().pageOnShow(this), swan.showLoading({
            title: "正在加载",
            mask: !0
        });
        var n = this, a = swan.getStorageSync("picker_address");
        a && (n.setData({
            address: a,
            name: a.name,
            mobile: a.mobile
        }), swan.removeStorageSync("picker_address"));
        var t = "";
    ...
    orderSubmit: function(a) {
        ...
        if (1 == e) {
            ...
            if (i.address_name = t.data.name, i.address_mobile = t.data.mobile, !t.data.shop.id) return void swan.showModal({
                title: "警告",
                content: "请选择门店",
                showCancel: !1
            });
        ...
        swan.showLoading({
            ...
            data: i,
            success: function(a) {
                swan.hideLoading(), 0 == a.code ? 1 == a.type ? swan.redirectTo({
                    ...
                    complete: function(a) {
                        "requestPayment:fail" != a.errMsg && "requestPayment:fail cancel" != a.errMsg ? "requestPayment:ok" == a.errMsg && swan.redirectTo({
                            url: "/pages/integral-mall/order/order?status=1"
                        }) : swan.showModal({
                            title: "提示",
                            content: "订单尚未支付",
