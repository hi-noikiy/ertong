...
Page({
    ...
    onReady: function() {},
    onShow: function() {
        var c = this, a = swan.getStorageSync("share_setting"), e = swan.getStorageSync("custom");
        c.setData({
            share_setting: a,
            custom: e
        }), app.request({
    ...
    formSubmit: function(a) {
        ...
        if (-1 != e.data.cash_max_day && (i = min(i, e.data.cash_max_day)), t) if (i < t) swan.showToast({
            title: "提现金额不能超过" + i + "元",
            image: "/images/icon-warning.png"
        }); else if (t < parseFloat(e.data.share_setting.min_money)) swan.showToast({
            ...
            if (0 == n || 1 == n || 2 == n || 3 == n) {
                if (0 == n || 1 == n || 2 == n) {
                    if (!(c = a.detail.value.name) || null == c) return void swan.showToast({
                        title: "姓名不能为空",
                        image: "/images/icon-warning.png"
                    });
                    if (!(o = a.detail.value.mobile) || null == o) return void swan.showToast({
                        title: "账号不能为空",
                        image: "/images/icon-warning.png"
                    });
                }
                if (2 == n) {
                    if (!(s = a.detail.value.bank_name) || null == s) return void swan.showToast({
                        title: "开户行不能为空",
                        image: "/images/icon-warning.png"
                    });
                } else var s = "";
                ...
                swan.showLoading({
                    ...
                    success: function(e) {
                        swan.hideLoading(), swan.showModal({
                            ...
                            showCancel: !1,
                            success: function(a) {
                                a.confirm && 0 == e.code && swan.redirectTo({
