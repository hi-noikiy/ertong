...

{
...

    {
        ...
        $order_max = 10000; //每次最多处理的团购个数，防止运行太久
        ...
        foreach ($parent_order_list as $parent_order) {
            ...
            foreach ($order_list as $i => $order) {
                ...
                if ($order->pay_type == 1) {
                    ...
                    $error_log->store_id = $order->store_id;
                    $error_log->order_sn = $order->order_no;
                    $error_log->refund_type = 1;
                    $error_log->reason = json_encode($res).'374';
                    $error_log->addtime = time();
                    $error_log->save();
