...
$name=$_GPC['name'];
$qs_id=$_GPC['qs_id'];
$where=" where xx.uniacid=:uniacid and xx.days like '%{$days}%' and xx.qs_id={$qs_id} and xx.state!=5 ";
$data[':uniacid']=$_W['uniacid'];
if($_GPC['time']){
    $start=strtotime($_GPC['time']['start']);
    $end=strtotime($_GPC['time']['end']);
    $where=" where  UNIX_TIMESTAMP(xx.days) >={$start} and UNIX_TIMESTAMP(xx.days)<={$end} and xx.qs_id={$qs_id} and xx.state!=5";
}
