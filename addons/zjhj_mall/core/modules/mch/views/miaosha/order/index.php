        ...
        <ul class="nav nav-tabs status">
            ...
            <li class="nav-item">
                <a class="status-item  nav-link <?= $status == 9 ? 'active' : null ?>"
                   href="<?= $urlManager->createUrl(array_merge([$_GET['r']], $condition, ['status' => 9])) ?>">待自提<?= $store_data['status_count']['status_9'] ? '(' . $store_data['status_count']['status_9'] . ')' : null ?></a>
            </li>
            <li class="nav-item">
                <a class="status-item  nav-link <?= $status == 10 ? 'active' : null ?>"
                   href="<?= $urlManager->createUrl(array_merge([$_GET['r']], $condition, ['status' => 10])) ?>">待评价<?= $store_data['status_count']['status_10'] ? '(' . $store_data['status_count']['status_3'] . ')' : null ?></a>
            </li>
            <li class="nav-item">
                            ...
                            <div>
                                ...
                                <?php if ($order_item['is_send'] == 1 && $order_item['is_cancel'] == 0 && $order_item['is_delete'] == 0) : ?>
                                    <a class="btn btn-sm btn-primary edit-address"
                                       data-index="<?= $k ?>"
                                       data-order-type="miaosha" href="javascript:">修改自提柜</a>
                                <?php endif; ?>
...
<script>
    ...
    $(document).on('click', '.update-address', function () {
        ...
        var href = '<?= $urlManager->createUrl(['mch/miaosha/order/update-order-address']) ?>';
        $.ajax({
            url: href,
            type: "post",
            data: {
