...
if($_GPC['id']){
	...
	if($res){
		message('设置成功',$this->createWebUrl2('sjgroupgoods',array('page'=>$_GPC['page'],'keywords'=>$_GPC['keywords'],'dishes_type'=>$_GPC['dishes_type'],'type_id'=>$_GPC['type_id'],'is_show2'=>$_GPC['is_show2'])),'success');
	}else{
		message('设置失败','','error');
	}
}
if($_GPC['op']=='delete'){
	...
	if($result){
		message('删除成功',$this->createWebUrl2('sjgroupgoods',array()),'success');
	}else{
		message('删除失败','','error');
	}
}
if($_GPC['op']=='play'){
	...
	if($res){
		message('设置成功',$this->createWebUrl2('sjgroupgoods',array('page'=>$_GPC['page'],'keywords'=>$_GPC['keywords'],'dishes_type'=>$_GPC['dishes_type'],'type_id'=>$_GPC['type_id'])),'success');
	}else{
		message('设置失败','','error');
	}
}
if($_GPC['op']=='tg'){
    ...
    if($res){
         message('通过成功！', $this->createWebUrl2('sjgroupgoods'), 'success');
        }else{
              message('通过失败！','','error');
        }
}
if($_GPC['op']=='jj'){
    ...
    if($res){
         message('拒绝成功！', $this->createWebUrl2('sjgroupgoods'), 'success');
        }else{
         message('拒绝失败！','','error');
        }
}

include $this->template('web/sjgroupgoods');
