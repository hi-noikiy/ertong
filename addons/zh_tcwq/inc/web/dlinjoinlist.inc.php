...
if($_GPC['op']=='delete'){
    ...
    if($res){
        message('删除成功',$this->createWebUrl2('dlinjoinlist',array('act_id' => $_GPC['act_id'])),'success');
    }else{
        message('删除失败','','error');
    }
}
if($_GPC['op']=='tg'){
    ...
    if($res){
        message('操作成功',$this->createWebUrl2('dlinjoinlist',array('act_id' => $_GPC['act_id'])),'success');
    }else{
        message('操作失败','','error');
    }
}
 ...
 // $input->SetTransaction_id($refundid);
   ...
   // var_dump($result);die;
    ...
    //更改订单操作
        ...
        message('退款成功',$this->createWebUrl2('dlinjoinlist',array('act_id' => $_GPC['act_id'])),'success');
    }else{
        message($result['result_code'],'','error');
}
}
include $this->template('web/dlinjoinlist');