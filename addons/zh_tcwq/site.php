...
$yuser=implode(',',$yuser);
$sql =" select id,user_id,user_name as name from ".tablename('zhtc_distribution')." where uniacid={$_W['uniacid']}  and user_id not in ({$yuser}) and user_name like '%{$_GPC['keywords']}%'";  
$user=pdo_fetchall($sql);
return json_encode($user);
}
