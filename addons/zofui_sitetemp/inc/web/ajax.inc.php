<?php 
	...
	elseif ($_GPC['op'] == 'savepage') {
		...
		if( $_GPC['bar']['bartype'] == 1 ) {
			...
			for ($i=0; $i < $_GPC['bar']['num']; $i++) { 
				...
				if( strpos($_GPC['bar']['data'][$i]['url'],'zofui_sitetemp/pages/page/page') !== false || empty( $_GPC['bar']['data'][$i]['url'] ) ){
		...
		$isset = pdo_get('zofui_sitetemp_bar',array('tempid'=>$tid),array('id'));

		if( !empty( $isset['id'] ) ){
			$resa = pdo_update('zofui_sitetemp_bar',$data,array('id'=>$isset['id']));
			$id = $isset['id'];
		}else{
			$resa = pdo_insert('zofui_sitetemp_bar',$data);
			$id = pdo_insertid();
		}
		if($resa){
	...
	// 查询链接
		...
		$temp['page'] = pdo_getall('zofui_sitetemp_page',array('tempid'=>$_GPC['tid'],'isinit'=>0),array('id','name'));
		...
		if( !empty( $alltemp ) ) {
			foreach ( $alltemp as &$v ) {
				if( $v['id'] != $temp['id'] ) {
					$v['page'] = pdo_getall('zofui_sitetemp_page',array('uniacid'=>$_W['uniacid'],'tempid'=>$v['id'],'isinit'=>0),array('id','name'));
	...
	// 查询模板的页面
		...
		$page = pdo_get('zofui_sitetemp_page',array('id'=>$_GPC['id']));
		...
		$page = pdo_get('zofui_sitetemp_page',array('id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']));

		if( empty( $page ) ) {
			Util::echoResult(201,'页面不存在');
		}

		pdo_delete('zofui_sitetemp_page',array('id'=>$page['id']));
	...
	// 使用模板
		...
		$all = pdo_getall('zofui_sitetemp_temp',array('uniacid'=>94));
		if( !empty( $all ) ) {
			foreach ($all as $v) {
				...
				$pages = pdo_getall('zofui_sitetemp_page',array('tempid'=>$v['id']));
				$bar = pdo_get('zofui_sitetemp_bar',array('tempid'=>$v['id']));
				$todata['pages'] = $pages;
				$todata['bar'] = $bar;
	...
	// 处理卡券
		...
		$allgood = pdo_getall('zofui_sitetemp_good');
		foreach ( $allgood as $v ) {
			...
			pdo_update('zofui_sitetemp_good',array('content'=>$params),array('id'=>$v['id']));
		}

		$allgood = pdo_getall('zofui_sitetemp_appoint');
		foreach ( $allgood as $v ) {
			$params = str_replace(array($_GPC['oldsite'],urlencode( $_GPC['oldsite'] ),htmlspecialchars( $_GPC['oldsite'] )), array($_GPC['newsite'],$_GPC['newsite'],$_GPC['newsite']), $v['content']);
			pdo_update('zofui_sitetemp_appoint',array('content'=>$params),array('id'=>$v['id']));
		}

		$allgood = pdo_getall('zofui_sitetemp_card');
		foreach ( $allgood as $v ) {
			$params = str_replace(array($_GPC['oldsite'],urlencode( $_GPC['oldsite'] ),htmlspecialchars( $_GPC['oldsite'] )), array($_GPC['newsite'],$_GPC['newsite'],$_GPC['newsite']), $v['content']);
			pdo_update('zofui_sitetemp_card',array('content'=>$params),array('id'=>$v['id']));
		}	

		$allgood = pdo_getall('zofui_sitetemp_product');
		foreach ( $allgood as $v ) {
			$params = str_replace(array($_GPC['oldsite'],urlencode( $_GPC['oldsite'] ),htmlspecialchars( $_GPC['oldsite'] )), array($_GPC['newsite'],$_GPC['newsite'],$_GPC['newsite']), $v['content']);
			pdo_update('zofui_sitetemp_product',array('content'=>$params),array('id'=>$v['id']));
		}

		$allgood = pdo_getall('zofui_sitetemp_article');
		foreach ( $allgood as $v ) {
			$params = str_replace(array($_GPC['oldsite'],urlencode( $_GPC['oldsite'] ),htmlspecialchars( $_GPC['oldsite'] )), array($_GPC['newsite'],$_GPC['newsite'],$_GPC['newsite']), $v['content']);
			pdo_update('zofui_sitetemp_article',array('content'=>$params),array('id'=>$v['id']));
		}
