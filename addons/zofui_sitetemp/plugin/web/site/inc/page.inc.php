...
//var_dump( $where );
		...
		$info = Util::getAllDataInSingleTable('zofui_sitetemp_page',$where,$_GPC['page'],10,' `id` DESC ',false,true,' id,name,tempid ','','',false);
		$list = $info[0];
		$pager = $info[1];
	...
	if( $_GPC['op'] == 'edit' ) {
		...
		$alltemppage = pdo_getall('zofui_sitetemp_page',array('tempid'=>$page['tempid']),array('id','name'));
	...
	if( $_GPC['op'] == 'add' ) {
		...
		if( empty( $page ) ){
			...
			pdo_insert('zofui_sitetemp_page',$indata);
		...
		$alltemppage = pdo_getall('zofui_sitetemp_page',array('tempid'=>$_GPC['tid']),array('id','name'));
		...
		if( $_GPC['opp'] == 'bar' ) {
			$page = pdo_get('zofui_sitetemp_page',array('uniacid'=>$_W['uniacid'],'tempid'=>$thistemp['id']));
			$page['params'] = iunserializer( $page['params'] );
		}
