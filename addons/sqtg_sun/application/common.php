...
//启动任务
function runTask($process_id){
    if (!$process_id){
        $process_id = uniqid();
    }
    global $_W;
    $url = $_W['sitescheme'].$_SERVER['HTTP_HOST']."/app/index.php?i=".$_W['uniacid']."&t=0&v=1.0&from=wxapp&c=entry&a=wxapp&do=Ctask|run1&m=sqtg_sun&process_id={$process_id}";
...
//        用于本地测试
        if($domain_a == 'ljh.yzdc.com:8080'||$domain_a='39.104.50.190'){
            return true;
        }
