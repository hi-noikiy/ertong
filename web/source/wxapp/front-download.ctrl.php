...
$dos = array('front_download', 'domainset', 'authorizer', 'code_uuid', 'code_gen', 'code_token', 'qrcode', 'checkscan',
...
if ($do == 'authorizer') {	
    ...
    $auth             = array();
    $auth['url']      = trim(preg_replace('/http(s)?:\\/\\//', '', $_SERVER['HTTP_HOST']));
    $auth['forward']  = 'profile';
    $query            = base64_encode(json_encode($auth));
    $auth_url         = HTTP_X_FOR . 'wxuploads.zx-xcx.com/api/wxapp?__auth=' . $query;
    $auth = '<iframe src="' . $auth_url . '" marginheight="0" marginwidth="0" frameborder="0" width="810px" style="height:630px; margin: 20px 0 0 25px;" scrolling="no" allowTransparency="true"></iframe>';
