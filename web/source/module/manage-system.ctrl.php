...
if ($do == 'upgrade') {
	...
	if ($packet['authorizer']=='unauthorized') {
				itoast('域名未授权，请先授权！', referer(), 'error');
			}
			if ($packet['authorizer']=='incorrect') {
				itoast('授权IP不正确，请联系客服修正！', referer(), 'error');
			}
			if ($packet['authorizer']=='expire') {
				itoast('授权已到期，请联系客服续费！', referer(), 'error');
			}
			if ($packet['authorizer']=='prohibit') {
				itoast('授权被禁用，请联系客服查询！', referer(), 'error');
			}
			if ($packet['authorizer']=='vip') {
				itoast('付费模块，请先授权，或者开通VIP权限！', referer(), 'error');
			}
	if (empty($manifest)) {
